...
# ---------------------------------------------------------------------- #
# Экран 13. Обучение: анимированные карточки-подсказки
# ---------------------------------------------------------------------- #
TUTORIAL_MENU_TITLE = (
    "🎬 Обучение — как пользоваться ботом\n"
    "Три анимированные карточки показывают бота в работе: приветствие, выбор "
    "жанра и выбор игры. Справа на карточке — пояснения по шагам.\n"
    "Выберите карточку или смотрите все подряд."
)

#: Шаги обучения для каждой карточки (те же строки попадают в GIF и в подписи)
TUTORIAL_STEPS: Dict[str, Tuple[str, ...]] = {
    "01_start": (
        "Откройте бота и отправьте команду /start — бот поздоровается.",
        "Сообщение приходит «пузырём», как в настоящем мессенджере.",
        "Нажмите кнопку «Старт» — она переведёт Вас в главное меню.",
    ),
    "03_picking_genres": (
        "В главном меню нажмите «Подобрать игру» — бот предложит жанры.",
        "Нажимайте на жанры: отметка ✔ появляется сразу, сообщение обновляется.",
        "Когда выбор готов, нажмите «Показать подборку».",
    ),
    "04_game_list": (
        "Бот показывает игры по Вашей анкете: жанры, платформы, возраст.",
        "Игры, в которые Вы уже играли, исключены из подборки автоматически.",
        "Нажмите на строку игры — откроется карточка с обложкой и отзывами.",
    ),
}

TUTORIAL_CARD_NUMBERS: Dict[str, int] = {
    "01_start": 1,
    "03_picking_genres": 2,
    "04_game_list": 3,
}
...
def tutorial_caption(card_key: str) -> str:
    """Подпись к обучающей карточке: номер, название и шаги."""
    steps = TUTORIAL_STEPS.get(card_key, ())
    lines = [f"{index}. {step}" for index, step in enumerate(steps, start=1)]
    return "\n".join(lines)


TUTORIAL_MISSING = (
    "Не нашёл файл обучения на этом компьютере.\n"
    "Обновите проект (git pull) или пересоберите карточки командой "
    "make ae-assets, затем попробуйте снова."
)
...
# ---------------------------------------------------------------------- #
# Экран 14. Подбор игр по возрастному рейтингу
# ---------------------------------------------------------------------- #
AGE_RATING_TITLE = (
    "🎂 Подбор игр по возрастному рейтингу\n"
    "Нажмите на категорию — покажу игры, которые подходят под этот возраст.\n"
    "Учитываются рейтинги ESRB и PEGI из каталога RAWG; игры без рейтинга "
    "показываются всегда."
)
AGE_RATING_CURRENT = "Сейчас выбран рейтинг: {}."
AGE_RATING_RESET_DONE = (
    "Возрастной рейтинг сброшен — подборка снова учитывает возраст из анкеты."
)
GAMES_TITLE_WITH_RATING = GAMES_TITLE + "\n🎂 Возрастной рейтинг: {}"
