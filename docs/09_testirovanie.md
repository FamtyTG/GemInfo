...
|--------|--------|---------------|
| `test_mockups.py` | 252 | генератор мокапов: 18 мокапов, SVG-разметка, соответствие текстов и кнопок коду бота, актуальность файлов в `docs/mockups/` |
| `test_screens.py` | 126 | экраны: текст, кнопки, переходы, состояние контекста, ошибки |
| `test_keyboards.py` | 97 | подписи кнопок, состав клавиатур, схема callback-данных и её разбор |
...
| `test_handlers.py` | 86 | регистрация обработчиков, все 34 callback-маршрута, ввод текста, ошибки, игнорирование «мусора» |
...
| `test_state.py` | 66 | `ContextScreen`, `UserContext` (переходы, неизменяемость), `StateStorage` |
...
| `test_app.py` | 43 | логирование, сборка приложения по слоям, запуск и остановка, `main`, `TelegramGateway` |
| `test_config.py` | 31 | `Settings.from_env`: переменные окружения, `.env`, значения по умолчанию, ошибки |
...
| `test_http_client.py` | 24 | HTTP-клиент: таймаут, коды ответа, JSON, `allow_404`, ошибки |
| `test_ae_assets.py` | 38 | генератор обучающих карточек: раскладка 16:9, раскадровка, тайминг кадров (60 fps), экспорт MP4, артефакты `docs/ae/` и ролики в `gamehunter/assets/tutorial/` |
| `test_tutorial.py` | 8 | видеообучение: ролики на диске, отправка на экранах 1/3/4 с подписями-шагами, устойчивость к отсутствию файла и ошибке отправки |
| `test_rating_screen.py` | 5 | Экран 13 «Возрастной рейтинг»: выбор категории, подмена возраста анкеты, сброс, неизвестный рейтинг |
| **Всего** | **1497** (1451 проходят, 46 пропускаются по условиям) | |

Инфраструктура тестов:

| Файл | Назначение |
|------|------------|
| `tests/fakes.py` | `FakeGamesProvider`, `FakeIpProvider`, `InMemoryProfileRepository`, `InMemoryLibraryRepository`, `FakeSession`/`FakeResponse` (подмена `requests`), `FakeGateway` (текст, фото, анимации), фабрики сущностей (`make_game`, `make_details`, `make_profile`, `make_played` …), `default_provider` — каталог из 7 игр |
| `tests/conftest.py` | фикстуры: `database` (SQLite в `tmp_path`), репозитории, сервисы, `games_provider`, `ip_provider`, `gateway`, `storage`, `screens` (все 18 экранов), `bot`, `handlers` |
...
* **Клавиатуры**: состав рядов и подписи, callback-данные каждой кнопки,
  `parse_callback` для всех 34 действий, неполные данные (`game:`) → `None`.
...
* **Экраны**: текст и кнопки каждого экрана, сохранение контекста, переходы,
  ...
  обучающие MP4-ролики приходят вместе с экранами 1, 3 и 4 с подписями из
  `texts.TUTORIAL_STEPS`; Экран 13 — выбор категории рейтинга, подмена возраста
  анкеты в подборке и сброс.
...
* **Сборка приложения**: `Application` связывает все три слоя, настройки
  применяются к сервисам, таблицы создаются, все 18 экранов присутствуют,
