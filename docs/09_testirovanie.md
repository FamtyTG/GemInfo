...
| `test_ae_assets.py` | 37 | генератор обучающих карточек: раскладка 16:9, раскадровка, тайминг кадров (60 fps), артефакты `docs/ae/` и GIF в `gamehunter/assets/tutorial/` |
| `test_tutorial.py` | 13 | Экран 13 «Обучение»: карточки на диске, отправка анимаций с подписями-шагами, маршруты `tut*`, ошибки отправки |
| `test_rating_screen.py` | 5 | Экран 14 «Возрастной рейтинг»: выбор категории, подмена возраста анкеты, сброс, неизвестный рейтинг |
| **Всего** | **1501** (1455 проходят, 46 пропускаются по условиям) | |
...
| `tests/conftest.py` | фикстуры: `database` (SQLite в `tmp_path`), репозитории, сервисы, `games_provider`, `ip_provider`, `gateway`, `storage`, `screens` (все 19 экранов), `bot`, `handlers` |
...
* **Экраны**: текст и кнопки каждого экрана, сохранение контекста, переходы,
  ...
  Экран 13 — отправка GIF с подписями из `texts.TUTORIAL_STEPS`, Экран 14 —
  выбор категории рейтинга, подмена возраста анкеты в подборке и сброс.
...
* **Сборка приложения**: `Application` связывает все три слоя, настройки
  применяются к сервисам, таблицы создаются, все 19 экранов присутствуют,
